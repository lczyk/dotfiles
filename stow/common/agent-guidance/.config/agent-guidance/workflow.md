...
- **capabilities the user can grant** the fence reads `AGENT_UNFENCE` -- comma-separated tokens -- from its own environment, set when the session is launched (`env AGENT_UNFENCE=branch claude`). four exist:
    ...
    - `remote` lifts the remote-env category: `ssh`, `scp`, `autossh`, `kubectl exec`, `gcloud compute ssh`. `git push` is a separate category and stays blocked -- lifting `remote` does not reach a git remote.
...
- **never ssh or work in remote environments** unless explicitly instructed to. ask before doing anything that crosses the local boundary. (the user can lift the fence for a session with `env AGENT_UNFENCE=remote`; the instruction still has to say which host and what for.)
