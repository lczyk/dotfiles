...
- **expensive non-file work gets a stamp** for rules whose output is not a file make can date-compare (container images, uploads), gate on `.stamp/<name>: FORCE | .stamp` with the change-detection (input hashing) inside the called script; `clean-<thing>` removes the stamps.
