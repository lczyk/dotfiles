...
the writing-style / voice rules ("lofi") live in `~/.claude/styles/lofi.md` -- injected every session by the lofi hooks in `settings.json` (SessionStart full text, UserPromptSubmit per-turn digest). edit the style there, not here.
