...
- **log command output to `/tmp/claude/log/`** for any command producing significant output (tests, builds, long pipelines), stream output to a file in `/tmp/claude/log/` so it persists for the session. preferred pattern:
    ```
